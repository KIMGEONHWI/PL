...
def term():
    result = factor()
    while pos < len(tokens) and (tokens[pos] == '*' or tokens[pos] == '/'):
        syntax_error() # throw error for '*' and '/' operator
    return result
...
def syntax_error(code=None):
    global err
    err = True
    if code and re.search(r'[^a-zA-Z0-9+\-*/();= \n]', code):
        print("Syntax Error!!")
...
while True:
    ...
    if code.strip() == '':
        blank_input_count += 1
        if blank_input_count == 2:  
            break
        continue
    else:
        blank_input_count = 0  

    if code[-1] != ';' or re.search(r'[^a-zA-Z0-9+\-*/();= \n]', code):
        print("Syntax Error!!")
        continue
